...
namespace $rootname$.Common.Pages
{
    /// <summary>
    /// Baseclass for $rootname$ pages that show a list of items
    /// </summary>
    public abstract class Base$rootname$ListPage<TTableEnum> : Base$rootname$Page
    {
        private const int COLUMN_NOT_FOUND = -1;

        public Base$rootname$ListPage(IWindsorContainer container, IWebDriver driver, string title, string code)
            : this(container, driver)
        {
            // Nothing to do here
        }

        public Base$rootname$ListPage(IWindsorContainer container, IWebDriver driver)
